...
def runDiaumpire(event, log_fh, progressData, cwd, DIAfiles, outDir):
    ...
    for i, filename in enumerate(filenames):
        ...
        if not (os.path.exists(mgf1_file) \
            ...
            free_gigs = psutil.virtual_memory().available / (1024*1000000)
            ...
            params = [
                ...
                "/opt/diatools/diaumpire_params.txt"
            ]
...
def buildDIAMatrix(\
    ...
    for i, DIA_filename in enumerate(successfull_DIA_filenames):
        ...
        pyprophet_proc = subprocess.Popen(pyprophet_cmd, cwd=cwd, stdout=log_fh, stderr=log_fh)

        while True:
            try:
                pyprophet_proc.wait(1)
                break
            except subprocess.TimeoutExpired:
                if event and event.kill_flag:
                    pyprophet_proc.kill()
...
if __name__=="__main__":
    ...
    parser.add_argument('--in-DDA-mzXML',
                        ...
                        required=False,
                        default=None,
                        help='MSMS files for DDA library in mzXML format')
    ...
    parser.add_argument('--in-DIA-mzML',
                        ...
                        required=False,
                        default=None,
                        help='MSMS DIA sample files in mzML format')
    ...
    parser.add_argument('--no-libfree', 
                        action='store_true',
                        dest='no_libfree',
                        required=False,
                        default=False,
                        help='Disable libfree method and do not generate spectral library from DIA files.')
    ...
    if args.in_dia_mzML:
       swaths, tswaths = create_swath_window_files(cwd, args.in_dia_mzML[0])
       swaths_min = swaths[0][0]
       swaths_max = swaths[-1][1]
       assert (swaths_min < swaths_max)
       if not args.no_libfree:
           pseudospectrafiles = runDiaumpire(None, log_fh, None, cwd, args.in_dia_mzML, "libfree")
    ...
    if not os.path.exists(db_filename) or not os.path.exists(decoy_db_file):
        build_database(None, log_fh, None, cwd, args.db, db_filename, decoy_db_file)
    else:
        print ("Database files exist already.")
    ...
    if args.use_msgf:
        if args.in_dda_mzXML:
            runMSGFPlus(decoy_db_file, args.in_dda_mzXML, "interact_msgf_pep.xml", args.threads)
            libmethod_pepXMLs.append("interact_msgf_pep.xml")
        if pseudospectrafiles:
            runMSGFPlus(decoy_db_file, pseudospectrafiles, "interact_msgf_pseudo_pep.xml", args.threads)
            libfreemethod_pepXMLs.append("interact_msgf_pseudo_pep.xml")
    ...
    if args.use_comet:
        if args.in_dda_mzXML:
            runComet(\
                ...
                None, \
                cwd, \
                args.comet_cfg_template, \
                decoy_db_file, args.in_dda_mzXML, \
                None, \
